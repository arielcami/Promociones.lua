...
local GIFT_BAG          = {give=true, id=51809, amount=2} --> Decide si regalar una bolsa o no, el ID de la bolsa y la cantidad.
...
podría recibir más promociones de las que se definen acá en una cuenta nueva.
...
local function GOSSIP (Player, mesg, send, intid, bool, boxtext) 
    Player:GossipMenuAddItem(0, mesg, send, intid, bool, boxtext) 
end
...
local function EQUIP (Player, build, class) ------------------------------------------------------------
    ...
    if GIFT_BAG.give then --> Bolsas de Regalo.
        local bagAmount = GIFT_BAG.amount
        if bagAmount >= 5 then bagAmount = 4 end --> Por si acaso se le ocurre a alguien poner más de 4 bolsas.
...
local function GIVE_SKILLS(Pl,C) --> Para ofrecer servicios extra como skills y objetos necesarios. La llamaremos más abajo.
    
    local SPELLS = {[1]={71,7386,355,2458,20252}, [2]={7328}, [4]={1515,883,2641,6991,982}, [64]={8071,3599,5394}, 
                    [256]={697,712,691}, [1024]={18960,5487,6795,6807}}

    local TOTEMS = {5175,5176,5177,5178}

    local function giveSpell(sp)    if not Pl:HasSpell(sp)  then Pl:LearnSpell(sp)  end end
    local function giveItem(it)     if not Pl:HasItem(it)   then Pl:AddItem(it)     end end

    for k, id in ipairs(SPELLS[C]) do giveSpell(id) end --> Spells les damos a todos.
    if C==64 then for i=1,4 do giveItem(TOTEMS[i]) end end --> Pero esto solo a los Chamanes
end ------------------------------------------------------------------------------------------------------------------------

local function WELCOME_MESSAGE(p,build) --> Para enviar un mensaje a todos los jugadores cuando alguien recibe la promoción.

    local race_male = {[1]="|cff00a2ffHumano", [2]="|cffff0000Orco", [4]="|cff00a2ffEnano", [8]="|cff00a2ffElfo de la Noche", [16]="|cffff0000No-Muerto",
        [32]="|cffff0000Tauren", [64]="|cff00a2ffGnomo", [128]="|cffff0000Trol", [512]="|cffff0000Elfo de Sangre", [1024]="|cff00a2ffDraenei"}

    local race_female = {[1]="|cff00a2ffHumana", [2]="|cffff0000Orco", [4]="|cff00a2ffEnana", [8]="|cff00a2ffElfa de la Noche", [16]="|cffff0000No-Muerta",
        [32]="|cffff0000Tauren", [64]="|cff00a2ffGnomo", [128]="|cffff0000Trol", [512]="|cffff0000Elfa de Sangre", [1024]="|cff00a2ffDraenei"}

    local class_male ={[1]="|CFFC69B6DGuerrero", [2]="|CFFF48CBAPaladín", [4]="|CFFAAD372Cazador", [8]="|CFFFFF468Pícaro", [16]="|CFFFFFFFFSacerdote",
        [32]="|CFFC41E3ACaballero de la Muerte", [64]="|CFF0070DDChamán", [128]="|CFF3FC7EBMago", [256]="|CFF8788EEBrujo", [1024]="|CFFFF7C0ADruida"}

    local class_female ={[1]="|CFFC69B6DGuerrera", [2]="|CFFF48CBAPaladín", [4]="|CFFAAD372Cazadora", [8]="|CFFFFF468Pícara", [16]="|CFFFFFFFFSacerdotisa",
        [32]="|CFFC41E3ACaballero de la Muerte", [64]="|CFF0070DDChamán", [128]="|CFF3FC7EBMaga", [256]="|CFF8788EEBruja", [1024]="|CFFFF7C0ADruida"}

    local ROLE = ""--> Declaramos ahora, para que no haya problemas de alcance.

    -- Hacemos algunas asignaciones necesarias.
    local class, race = p:GetClassMask(), p:GetRaceMask()
    if build==100 then ROLE = "ability_warrior_defensivestance" end 
    if build==200 then ROLE = "ability_dualwield"               end
    if build==300 then ROLE = "spell_fire_firebolt"             end
    if build==400 then ROLE = "spell_holy_greaterheal"          end   

    --Operadores Ternarios: Elegimos las tablas que vamos a usar según el Género, y la letra "o" o "a" según corresponda
    local gen = p:GetGender()==0 and 1 or 2
    local CLASS = gen==1 and class_male or class_female
    local RACE  = gen==1 and race_male  or race_female

    local function timedMessage(ev, del, r, Pla)
        local str = string.format("|cff00ff00%s, %s %s|cff00ff00, se ha unido al servidor! Rol: |r|TInterface\\Icons\\%s:20:20:0|t", 
        Pla:GetName(), RACE[race], CLASS[class], ROLE)
        SendWorldMessage(str)
    end
    p:RegisterEvent(timedMessage, 1500) -- Establecemos que se ejecute en 1.5 segundos.
end ------------------------------------------------------------------------------------------------------------------------
...
local i,sub = { -- Iconos de ramas de talentos de clase, al fondo están el de Promo y Teletransporte a Dalaran.
    ...
    [5] = "|TInterface\\Icons\\spell_arcane_teleportdalaran:45:45:-24|t", --> Teleporte a Dalaran 
}, '|TInterface\\Icons\\%s:45:45:-24|t' --> String para hacer sustitución.

    local function MESSAGE(target, text) target:SendBroadcastMessage(text) end

local function Hello (E,P,U)    

    if SCRIPT_MODULE then 

        -- Consultas SQL para obtener información sobre las promociones. Solo declaración.
        local getPromosPerPlayer        = string.format("SELECT `promos` FROM `aa_promotion` WHERE `pguid` = %d",       P:GetGUIDLow()      )
        local getSumPromosPerAccount    = string.format("SELECT SUM(promos) FROM `aa_promotion` WHERE `account` = %d",  P:GetAccountId()    )
        local getSumPromosPerIP         = string.format("SELECT SUM(promos) FROM `aa_promotion` WHERE `ip` = '%s'",     P:GetPlayerIP()     ) 
        ...
        if CHECK_CHARACTER:GetInt8(0) ~= 0 then --> El jugador ya recibió una promoción.
            ...
            MESSAGE(P,"|cffff0000Este personaje ya fue promocionado.")

            if P:GetLevel() >= PROMO_LEVEL then GOSSIP(P, i[5].."Aceptar teletransporte a Dalaran", 555, 0) P:GossipSendMenu(502, U) end
            U:PerformEmote(274)
            ...
            if CHECK_ACCOUNT:GetInt8(0) ~= 0 then --> El jugador ya recibió promo con otro personaje de la misma cuenta.
                ...
                if CHECK_ACCOUNT:GetInt8(0) >= PROMOS_PER_ACCOUNT then --> El jugador llegó al límite de promos por cuenta.
                    ...
                    MESSAGE(P,"|cffff0000Has llegado al límite de promociones en una misma cuenta.")

                    if P:GetLevel() >= PROMO_LEVEL then GOSSIP(P, i[5].."Aceptar teletransporte a Dalaran", 555, 0) P:GossipSendMenu(503, U) end
                    U:PerformEmote(274)
                    return --> CAAAASO CERRAAAADOOO WOO WOO! 
                else
                ...
                if CHECK_IP:GetInt8(0) >= PROMOS_PER_IP then -- El jugador usó todas las promos disponibles.
                    ...
                    MESSAGE(P,"|cffff0000Agotaste todas las promociones disponibles.")

                    if P:GetLevel() >= PROMO_LEVEL then GOSSIP(P, i[5].."Aceptar teletransporte a Dalaran", 555, 0) P:GossipSendMenu(504, U) end
                    U:PerformEmote(274)
                    return --> CAAAASO CERRAAAADOOO WOO WOO!
...
RegisterCreatureGossipEvent(NPC_ID, 1, Hello) ---------------------------------------------------------------------------------------

local function Click (event, P, U, S, I) 

    if SCRIPT_MODULE then

        local C, N, IP, AC = P:GetClassMask(), P:GetName(), P:GetPlayerIP(), P:GetAccountId()
        ...
        if S>=100 and S<=400 then --> Se accede aquí en todas las BUILDS.
            P:SetLevel(PROMO_LEVEL)
            LEARN_WEAPONS(P,C)
            EQUIP(P,S,C)            
            P:ModifyMoney(GOLD * 10000)

            if TEACH_RIDING then local ridingSpells = {33388, 33391, 34090, 34091, 54197} for i=1,5 do P:LearnSpell(ridingSpells[i]) end end

            if (C==1) or (C==2) or (C==4) or (C==64) or (C==256) or (C==1024) then GIVE_SKILLS(P, C) end --> Las demás clases no necesitan skills extra.                

            P:GossipComplete()
            CharDBExecute("UPDATE aa_promotion SET promos = promos + 1 WHERE pguid = "..P:GetGUIDLow()) --> Se registra la promoción como un +1 en la columna promos.
            U:PerformEmote(4)
            WELCOME_MESSAGE(P,S)
        end
    end
end    
RegisterCreatureGossipEvent(NPC_ID, 2, Click)
...
local function LogIn(e, P) --> Al logear.
    ...
    if DK_HELPER and (P:GetClassMask()==32) and (P:GetLevel()==55) then --> Solo para DKs recién nacidos.
        ...
        P:RegisterEvent(delayed, 3000) --> Se ejecuta pasados 3 segundos.
    end
end     
